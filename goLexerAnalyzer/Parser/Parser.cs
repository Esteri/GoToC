	...
	public class EarleyParser
	{
		...
		public void Parse(List<Token> tokens)
        {
			...
			if ((tokens[0].Lexem == "import") && (tokens[1].Lexem == "fmt")) {
				...
				S = Init(tokens.Count);
				// â I0 äîáàâèëè ïðîäóêöèè àêñèîìû ñ òî÷êîé â ïîçèöèè 0
				...
				// òåïåðü íàäî ïðîéòè ïî èòåìàì â I0 è çàïèñàòü ïðàâèëà äëÿ íåòåðìèíàëîâ
				int k = 0;
				do {
					for(int i = 0; i < S[k].Count; i++) {
						...
						if(!item.IsComplete()) {
							if(item.NextWord is Terminal) {
								...
								// ðàçðàñòàíèå äåðåâà êîãäà òî÷êà íà êîíå÷íîé ïîçèöèè
								//if (k == 0)
								//	Predictor(S[0], S[0]);
								Predictor(item, k, k);
								//S[k].Print();
        ...
        {
			// òî÷êà âõîäà ðåêóðñèâíîãî àëãîðèòìà ðàçáîðà
			int j = S.Length - 1;
		...
		private void Finder(Item item, int stateNumber)
        {
			int i = item.Table;
			int j = stateNumber;
			reduct = item.Production.Number.ToString() + " " + reduct; // äîáàâëåíèå ïðàâèëà ê ðàçáîðó
